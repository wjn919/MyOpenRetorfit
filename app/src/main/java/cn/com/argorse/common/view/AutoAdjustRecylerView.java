...
public class AutoAdjustRecylerView extends RecyclerView {
	...
	public void checkAutoAdjust(int position) {
		...
		int lastvisiableposition = ((LinearLayoutManager) getLayoutManager())
				.findLastVisibleItemPosition();
		if (position == (firstvisiableposition + 1)
				|| position == firstvisiableposition) {
			// 当前位置需要向右平移
			leftScrollBy(position, firstvisiableposition);
		} else if (position == (lastvisiableposition - 1)
	...
	private void leftScrollBy(int position, int firstvisiableposition) {
		...
		if (leftChild != null) {
			...
			int startleft = leftx;
			int endleft = position == firstvisiableposition ? leftChild
					.getWidth() : 0;
	...
	private void rightScrollBy(int position, int lastvisiableposition) {
		...
		if (rightChild != null) {
			...
			int startright = dx;
			int endright = position == lastvisiableposition ? -1
					* rightChild.getWidth() : 0;
